#ifndef QweakSimFieldMap_h
#define QweakSimFieldMap_h
...
class QweakSimFieldMap {
  ...
  public: // constructors and destructor
    ...
    QweakSimFieldMap(const std::vector<coord_t>& min,
  ...
  public: // public methods
    ...
    void SetDimensions(const unsigned int ndim) {
      if (ndim == 0) {
        mycerr << "QweakSimFieldMap::SetDimensions: "
    ...
    void SetMinimumMaximumStep(const std::vector<coord_t>& min,
      ...
      for (unsigned int i = 0; i < value_n; i++) {
        try {
          ...
          mycerr << "QweakSimFieldMap:SetMinimumMaximumStep: "
    ...
    bool Set(const coord_t& coord, const value_t& value) {
      if (value_n != 1) {
        mycerr << "QweakSimFieldMap::Set(coord_t&,value_t&): "
    ...
    bool Set(const unsigned int linear_index, const value_t& value) {
      if (value_n != 1) {
        mycerr << "QweakSimFieldMap::Set(unsigned int,value_t&): "
               << "only valid for one-dimensional values." << myendl;
        return false;
      }
    ...
    value_t GetValue(const coord_t& coord) const {
      if (value_n != 1) {
        mycerr << "QweakSimFieldMap::GetValue(coord_t&): "
    ...
    bool GetValue(const coord_t* coord, double& value) const {
      if (value_n != 1) {
        mycerr << "QweakSimFieldMap::GetValue(coord_t*,value_t&): "
               << "only valid for one-dimensional fields." << myendl;
        return false;
      }
    ...
    bool WriteTextFile(const std::string& filename) const {
      ...
      if (! file.is_open()) {
        mycerr << "QweakSimFieldMap::WriteTextFile(string&): "
    ...
    bool ReadTextFile(const std::string& filename) {
      ...
      if (! file.is_open()) {
        mycerr << "QweakSimFieldMap::ReadTextFile(string&): "
               << "unable to open file " << filename << "." << myendl;
        return false;
      }
      if (! ReadText(file)) {
  ...
  private: // private methods
    ...
    value_t Get(const unsigned int* cell_index) const {
      if (value_n != 1) {
        mycerr << "QweakSimFieldMap::Get(unsigned int*): "
               << "only valid for one-dimensional fields." << myendl;
        return 0;
      }
...
inline bool QweakSimFieldMap<value_t,value_n>::Linear(
	const coord_t* coord,
	value_t* value) const
{
  ...
  for (unsigned int offset = 0; offset < (1U << fNDim); offset++) {
    ...
    if (! Get(Index(cell_index,offset), neighbor)) {
      if (fDebug >= kDebug) {
        mycout << "QweakSimFieldMap::Linear(coord_t*,value_t*): "
 ...
 */
template <class value_t, unsigned int value_n>
inline bool QweakSimFieldMap<value_t,value_n>::NearestNeighbor(
	const coord_t* coord,
	value_t* value) const
{
  // Get nearest cell
 ...
 */
template <class value_t, unsigned int value_n>
inline bool QweakSimFieldMap<value_t,value_n>::Check(const coord_t* coord) const
{
  for (unsigned int dim = 0; dim < fNDim; dim++) {
    if (fWrap[dim] == 0 && (coord[dim] < fMin[dim] || coord[dim] > fMax[dim])) {
      if (fDebug >= kDebug) {
        mycout << "QweakSimFieldMap::Check(coord_t*): "
 ...
 */
template <class value_t, unsigned int value_n>
inline bool QweakSimFieldMap<value_t,value_n>::Check(const unsigned int* cell_index) const
{
  for (unsigned int dim = 0; dim < fNDim; dim++) {
    if (cell_index[dim] >= fSize[dim]) {
      if (fDebug >= kDebug) {
        mycout << "QweakSimFieldMap::Check(unsigned int*): "
 ...
 */
template <class value_t, unsigned int value_n>
inline bool QweakSimFieldMap<value_t,value_n>::Check(const unsigned int linear_index) const
{
 ...
 */
template <class value_t, unsigned int value_n>
inline unsigned int QweakSimFieldMap<value_t,value_n>::Index(
 ...
 */
template <class value_t, unsigned int value_n>
inline void QweakSimFieldMap<value_t,value_n>::Cell(
	const coord_t coord,
 ...
 */
template <class value_t, unsigned int value_n>
inline void QweakSimFieldMap<value_t,value_n>::Coord(
	const unsigned int* cell_index,
	coord_t* coord) const
{
  for (unsigned int dim = 0; dim < fNDim; dim++)
 ...
 */
template <class value_t, unsigned int value_n>
inline bool QweakSimFieldMap<value_t,value_n>::WriteText(std::ostream& stream) const
{
 ...
 */
template <class value_t, unsigned int value_n>
inline bool QweakSimFieldMap<value_t,value_n>::ReadText(std::istream& stream)
{
  // Informational message
  ...
  stream >> fNDim >> n;
  if (n != value_n) {
    mycerr << "QweakSimFieldMap::ReadText(istream&): "
 ...
 */
template <class value_t, unsigned int value_n>
inline bool QweakSimFieldMap<value_t,value_n>::WriteBinaryFile(const std::string& filename) const
{
  std::ofstream file(filename.c_str(), std::ios::binary);
  if (! file.is_open()) {
    mycerr << "QweakSimFieldMap::WriteBinaryFile(string&): "
           << "unable to open file " << filename << "." << myendl;
    return false;
  }
  // Informational message
 ...
 */
template <class value_t, unsigned int value_n>
inline bool QweakSimFieldMap<value_t,value_n>::ReadBinaryFile(const std::string& filename)
{
  std::ifstream file(filename.c_str(), std::ios::binary);
  if (! file.is_open()) {
    mycerr << "QweakSimFieldMap::ReadBinaryFile(string&): "
           << "unable to open file " << filename << "." << myendl;
    return false;
  }
  // Informational message
