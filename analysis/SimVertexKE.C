...
//  QweakSimG4_Tree->Add(Form("/cache/mss/home/vmgray/rootfiles/MD9_Oct1_%s_%.2Fcm/*.root", posx.c_str(), anglex));
  ...
  for (size_t i = 0; i<KE.size();i++)
  {
     ...
     for (size_t j = 0; j<KE[i].size();j++)
     {
       ...
       KE[i][j]= new TH1D (Form("KE[%d][%d]",i,j),Form("Sim KE vertex for Octant number %d, set %d",i,j),4000,0.0,2000.0);
  ...
  {
     KE_tot[j] = new TH1D (Form("KE_tot[%d]",j),Form("Sim KE vertex for Octant number %d",j),4000,0.0,2000.0);
 ...
 //define a histogram to store all the means of the n histogram chunk 
  ...
  {
     h_KE_mean[j] = new TH1D (Form("h_KE_mean[%d]",j),Form("Mean Sim KE value distribution from the %d chunks for Octant number %d", n,j),4000,0.0,2000.0);
