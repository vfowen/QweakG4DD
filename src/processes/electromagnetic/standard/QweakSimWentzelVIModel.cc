				  ...
				  const G4DataVector& cuts)
{
  ...
  */
  ...
  if(useSecondMoment && IsMaster() && table) {
    ...
    //G4cout << "### QweakSimWentzelVIModel::Initialise: build 2nd moment table "
    //	   << table << G4endl;
    fSecondMoments =  
...
void QweakSimWentzelVIModel::InitialiseLocal(const G4ParticleDefinition*, 
				       G4VEmModel* masterModel)
{
  fSecondMoments = static_cast<QweakSimWentzelVIModel*>(masterModel)
				   ...
				   G4double /*safety*/)
{
  ...
  */
  do {
	...
	if(modifyTrajectory){
	  ...
	  if( _prob < amplitude ){
	    ...
	    temp.setX(sint * cos(phi));
	    temp.setY(sint * sin(phi));
	  }
	  if(debugPrint)
	...
	if(debugPrint){
	  ...
	  G4cout<<"\t"<<amplitude<<"\t"<<eEnergy<<"\t"<<acos(cost)*180/3.1415<<"\t"<<phi*180/3.1415
  ...
  if(debugPrint){
    G4cout<<__PRETTY_FUNCTION__<<G4endl;
    G4cout<<"\tlocal new dir RTP : "<<dir.getR()<<" "<<dir.getTheta()/ CLHEP::deg<<" "<<dir.getPhi()/ CLHEP::deg<<G4endl;
  }
  //FIXME
  ...
  //FIXME
  if(debugPrint){
    G4cout<<"\tcth, th, phi old.angle(new):" << cost << " " << acos(cost)/ CLHEP::deg << " " << phi/ CLHEP::deg << " " <<oldDirection.angle(dir)/ CLHEP::deg << G4endl;
    G4cout<<"\told dir: R th phi "<<oldDirection.getR()<<" "<<oldDirection.getTheta()/ CLHEP::deg<<" "<<oldDirection.getPhi()/ CLHEP::deg<<G4endl;
    G4cout<<"\tnew dir: R th phi "<<dir.getR()<<" "<<dir.getTheta()/ CLHEP::deg<<" "<<dir.getPhi()/ CLHEP::deg<<G4endl;
  }
  //FIXME
...
G4double QweakSimWentzelVIModel::ComputeTransportXSectionPerVolume(G4double cosTheta)
{
...
G4double QweakSimWentzelVIModel:: ComputeSecondMoment(const G4ParticleDefinition* p,
						G4double kinEnergy)
{
  G4double xs = 0.0;
...
void QweakSimWentzelVIModel::SetSingleScatteringFactor(G4double val)
{
  if(val > 0.05) {
    ssFactor = val;
